...
global origin, pos, old_pos, elbow_pos, end_theory, delta, direction
...
def rec(s):
    ...
    while True:
        try:
            msg = s.recv()
            delta_ = [float(n) for n in msg.decode('utf-8').split(",")]
            delta = np.array([delta_[0] * -1, delta_[1], delta[2]])
...
def new_frame():
    global pos, old_pos, elbow_pos, end_theory, delta, moving, direction, ser, angles, s
    ...
    if moving:
        pos = np.add(old_pos, np.array([delta[0],delta[1]])*scale)
...
s.bind('tcp://127.0.0.1:'+input("Port: "))
#s.setsockopt(zmq.CONFLATE, 1)
Thread(target=rec,args=(s,)).start()
